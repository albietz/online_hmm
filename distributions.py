...
class Gaussian(Distribution):
    ...
    def new_sufficient_statistics(self, x, cluster_id, K):
        return GaussianSufficientStatistics(x, cluster_id, K, self.dim)

    def online_max_likelihood(self, rho_obs, phi):
        s0 = rho_obs.rho0.dot(phi)
        self.mean = rho_obs.rho1.dot(phi) / s0
        self.cov = rho_obs.rho2.dot(phi) / s0 - self.mean[:,nax] * self.mean
...
class GaussianSufficientStatistics(SufficientStatistics):
    def __init__(self, x, cluster_id, K, size):
        super(GaussianSufficientStatistics, self).__init__(cluster_id, K)
        # 1{Z_t = i}
